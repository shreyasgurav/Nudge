...
function Phone({ children }: { children: React.ReactNode }) {
  ...
  return (
    // Scale the phone to fit available height on smaller screens.
    // We use a CSS custom property trick: the outer div is 300px wide but the
    // inner frame scales down via transform-origin so nothing clips.
    <div className="relative w-[300px] max-w-full" style={{ transformOrigin: "top center" }}>
      {/* Left side buttons */}
      ...
      <div className="relative rounded-[3rem] bg-gradient-to-br from-zinc-500 via-zinc-700 to-zinc-600 p-[3px] shadow-2xl">
        <div className="rounded-[2.85rem] bg-black p-[9px]">
          <div className="relative overflow-hidden rounded-[2.3rem] bg-black" style={{ height: "min(640px, calc(100dvh - 3.75rem - env(safe-area-inset-top) - 10rem))" }}>
            {/* Dynamic Island */}
...
export default function CampaignPreview(props: CampaignPreviewProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      {/* Tab switcher — Nudge pill style */}
      <div
        className="inline-flex rounded-full p-1 gap-0.5"
        style={{ background: "#f0f0ec", border: "1px solid #e8e8e4" }}
      >
        {tabs.map((t) => (
          <button
            key={t.key}
            ...
            style={{
              background: activeTab === t.key ? "#6B9FE8" : "transparent",
              color: activeTab === t.key ? "white" : "#7a7a72",
              boxShadow:
                activeTab === t.key
                  ? "0 2px 6px rgb(107 159 232 / 0.3)"
                  : "none",
            }}
          >
            {t.label}
